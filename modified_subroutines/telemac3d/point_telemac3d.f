...
!>   Allocate variables for MCPBE flocculation model
      ...
      IF (FLOC .AND. FLOC_TYPE.EQ.3) THEN
        ...
        IF (MCPBE_VER .GE. 2) THEN
          CALL BIEF_ALLVEC(1, FLOCMIC_DIA,'FMEDIA',IELM3,1,1,MESH3D)
        ...
        ELSE
          CALL BIEF_ALLVEC(1, FLOCMIC_DIA,'FMEDIA',0,1,0,MESH3D)
          CALL BIEF_ALLVEC(1, FLOCMEG_DIA,'FMEDIA',0,1,0,MESH3D)
          CALL BIEF_ALLVEC(1, FLOCMEG_DEN,'FMEDEN',0,1,0,MESH3D)
        ENDIF
      ELSE
        CALL BIEF_ALLVEC(1, FLOCMIC_DIA,'FMEDIA',0,1,0,MESH3D)
        CALL BIEF_ALLVEC(1, FLOCMAC_DIA,'FMEDIA',0,1,0,MESH3D)
        CALL BIEF_ALLVEC(1, FLOCMAC_DEN,'FMEDEN',0,1,0,MESH3D)
        CALL BIEF_ALLVEC(1, FLOCMEG_DIA,'FMEDIA',0,1,0,MESH3D)
        CALL BIEF_ALLVEC(1, FLOCMEG_DEN,'FMEDEN',0,1,0,MESH3D)
...
!
     ...
     &   .AND.INCLUS(COUPLING,'TOMAWAC')) THEN
...
!         SIZE OF TRAV2 MUST BE GREATER THAN 13+NTRAC
      ...
      ENDIF
!
!     S3D_LAYTHI: FROM 38+1+NTRAC   TO 38+NTRAC+S3D_NCOUCH
!
      ...
      ENDIF
!
      CALL ADDBLO(VARSOR,S3D_PVSCO)           ! 39+NTRAC+S3D_NCOUCH
      CALL ADDBLO(VARSOR,S3D_PVSNCO)          ! 40+NTRAC+S3D_NCOUCH
!
!

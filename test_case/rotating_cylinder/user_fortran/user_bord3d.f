...
!
      INTEGER K,NP,IBORD
      DOUBLE PRECISION PI, UMOD, THETA, XI, YI, RPM, OMEGA, RADIUS

      PI = ACOS(-1.D0)	  
      RPM = MIN(100.D0,1.D-1*AT)
      OMEGA = RPM*2.D0*PI/60.D0
      RADIUS = 0.05D0
      UMOD = OMEGA*RADIUS
      WRITE(LU,*) 'TIME:',AT,'RPM:',RPM,'U',UMOD
      DO K = 1,NPTFR2
        IF(LIUBOL%I(K).EQ.KENTU .AND. LIVBOL%I(K).EQ.KENTU) THEN
          DO NP = 1, NPLAN
            IBORD = (NP-1)*NPTFR2+K
            XI = MESH3D%X%R(NBOR3%I(IBORD))
            YI = MESH3D%Y%R(NBOR3%I(IBORD))
            IF (XI.GT.0.D0) THEN
              THETA = ATAN(YI/XI)
              UBORL%R(IBORD) = -UMOD*SIN(THETA)
              VBORL%R(IBORD) =  UMOD*COS(THETA)
              WBORL%R(IBORD) = 0.D0
            ELSEIF (XI.LT.0.D0) THEN
              ...
              VBORL%R(IBORD) =  UMOD*COS(THETA)
              WBORL%R(IBORD) = 0.D0
            ELSEIF (XI.EQ.0.D0 .AND. YI.GT.0.D0) THEN
              UBORL%R(IBORD) = -UMOD
              VBORL%R(IBORD) = 0.D0
              WBORL%R(IBORD) = 0.D0
            ELSEIF (XI.EQ.0.D0 .AND. YI.LT.0.D0) THEN
              UBORL%R(IBORD) = UMOD
              VBORL%R(IBORD) = 0.D0
              WBORL%R(IBORD) = 0.D0
            ENDIF

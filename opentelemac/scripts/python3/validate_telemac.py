...
import sys
import time
from os import path, walk, chdir, remove, environ, sep, listdir
...
from execution.wait_for_jobs import check_job_slurm
...
def run_validation_python_slurm(cfg, options, report, xcpts):
    ...
    jobid_file = path.join(environ['HOME'], "valid_run_{}.log".format(CFGS.cfgname))
    ...
    if options.hpc_queue == '':
        raise TelemacException("Option --queue is mandatary with --vnv-mode=slurm")
    ...
    new_list_files = []
    for py_file in list_files:
        # Getting short name
        # Need this because on some cluster the path is not the same from
        # command line (in failed) vs from python (in list file))
        # <root>/examples/telemac2d/gouttedo/vnv_goutte.py
        # wil return telemac2d/gouttedo/vnv_gouttedo
        short_name = sep.join(path.split(py_file)[:-2])[:-3]

        failed = crashed['timeout'] + crashed['failed']
        if failed != []:
            for fail in failed:
                if short_name in fail:
                    continue
    ...
    for ddir in jobs_ini.values():
        ...
        for ffile in listdir(run_dir):
            if ffile.endswith(".out"):
                with open(path.join(run_dir, ffile), 'r', encoding='utf-8') as f:
                    print(f.read())
...
a certain rank, and a certain tag'''))
    ...
    parser.add_argument( \
        ...
        choices=['mpi', 'slurm'],
        help=""" Defines validation execution mode (does not work for notebooks):
- mpi: Normal sequential run of all the validation Python files.
- slurm: Will run validation in 'cluster' mode.
         This means that first valdiation will be run with --vnv-run to submit all the jobs to the scheduler.

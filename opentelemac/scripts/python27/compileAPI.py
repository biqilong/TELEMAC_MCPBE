...
def get_api_incs_flags(cfgs, cfgname):
    ...
    incs_flags = cfgs[cfgname].get('incs_all','').replace('<root>',
                                                   cfgs[cfgname]['root'])\
                                          .replace('\n', ' ')
...
def generate_api(cfgs, cfgname):
    ...
    obj_dir = path.join(cfgs[cfgname]['root'], 'builds', cfgname, 'obj')
    ...
    mycopy(path.join(lib_dir, 'mascaret'+dyn_ext), \
...
def compile_api(cfgs, cfgname, silent, fcompiler="gnu95", compiler="gnu95", static=False):
    ...
    for short in ['t2d', 'sis', 't3d', 'art', 'wac']:
        ...
        skip_source += 'get_var_size_'+short+'_d set_boolean_'+short+'_d set_double_'+short+'_d '
        skip_source += 'set_integer_'+short+'_d set_string_'+short+'_d '
        skip_source += 'get_double_array_'+short+'_d get_integer_array_'+short+'_d '
        skip_source += 'set_double_array_'+short+'_d set_integer_array_'+short+'_d '

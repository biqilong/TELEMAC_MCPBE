...
class ApiModule(object):
        ...
        if recompile:
            ...
            if user_fortran is not None:
                ...
                user_fortran_lib_path = \
                    path.join(os.getcwd(),\
                              'libuser_fortran'+cfgs[cfgname]['sfx_lib'])
                ...
                if key == 'GEOMETRY FILE':
                    ...
                    for i in range(self.ncsize):
                        shutil.copyfile(ffile,\
                            ffile+'{0:05d}-{1:05d}'.format(self.ncsize-1, i))
    ...
    def concatenation_step(self):
        ...
        value, _ = getKeyWord('GEOMETRY FILE', cas, dico, frgb)
        geo_file = value[0].strip("'")
        geo_fmt = get_file_format('GEOMETRY FILE', cas)

        # Get the name of the boundary conditions file
        value, _ = getKeyWord('BOUNDARY CONDITIONS FILE', cas, dico, frgb)
        cli_file = value[0].strip("'")

        # Get name of the geometry file
        value, _ = getKeyWord('NUMBER OF HORIZONTAL LEVELS', cas, dico, frgb)
    ...
    def run_all_time_steps(self):
        ...
        for itime in xrange(ntimesteps):
            self.logger.debug('%d: beginning run_timestep %d', self.rank, itime)
    ...
    def get_mesh(self):
        ...
        for i in xrange(self.nbnodes):
            self.coordx[i] = self.get('MODEL.X', i=i,
                                      global_num=not self.parallel_run)
            self.coordy[i] = self.get('MODEL.Y', i=i,
                                      global_num=not self.parallel_run)
    ...
    def get(self, varname, i=0, j=0, k=0, global_num=True):
        ...
        """
        value = None
        vartype, _, ndim, _, _, _, _, _, self._error = self.get_var_type(varname)
    ...
    def get_on_polygon(self, varname, poly):
        ...
        for i, point in enumerate(points_in_poly):
            res[i] = self.get(varname, i=point,
                              global_num=not self.parallel_run)

...
try:
    ...
    print( "MEDLoader unavailable boundary elements will "\
...
> converter.py --refine coarse.slf -b coarse.cli -o coarsex4.slf
   ...
   # the silent option define if display stbtel information
   parser.add_argument(\
      "-s","--silent",action="store_true",dest="silent",default=False,
      help="disable stbtel output information")
   # the debug mode option
   parser.add_argument(\
      "--debug",action="store_true",dest="debug",default=False,
      help="Enable debug mode which displays more information during run time")
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# ~~~~ Identifying input and output information ~~~~~~~~~~~~~~~~~~~
   #if len(args) < 1:
   #   parser.print_help()

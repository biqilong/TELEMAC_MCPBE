...
def parseConfig_CompileTELEMAC(cfg,rescan,bypass):
   ...
   #
   get,tbd = getFolders_ModulesTELEMAC(cfgTELEMAC['root'],rescan,bypass)
   ...
   get,tbr = parseUserModules(cfg,cfgTELEMAC['MODULES'])
   get = get.split()
   ...
   for mod in tbd:
      if mod in get and mod != 'mascaret':
         for fle in tbd[mod]: remove(fle)

   cfgTELEMAC['COMPILER'].update({'MODULES':get})
   cfgTELEMAC['COMPILER'].update({'REBUILD':tbr})
   for mod in get:
...
def parseConfig_TranslateTELEMAC(cfg):
   ...
   get,tbd = getFolders_ModulesTELEMAC(cfgTELEMAC[cfg]['root'],False,False)
   cfgTELEMAC[cfg].update({'MODULES':get})
   cfgTELEMAC[cfg].update({'COMPILER':{}})
   # Get cmplr_mod: user list of module
   get,tbr = parseUserModules(CONFIGS[cfg],cfgTELEMAC[cfg]['MODULES'])
...
def parseConfig_DoxygenTELEMAC(cfg):
   ...
   # identified by matching the directory structure to the template
   # teldir\module_name\*telver\
   get,tbd = getFolders_ModulesTELEMAC(cfgTELEMAC['root'],False,False)
   cfgTELEMAC.update({'MODULES':get})
...
def parseConfig_ValidateTELEMAC(cfg):
   ...
   get,tbr = parseUserModules(cfg,cfgTELEMAC['MODULES'])
   cfgTELEMAC.update({'REBUILD':tbr})
   # Removing python3 examples folder
   if 'python3' in examples:
      examples.remove('python3')
   ...
   for moddir in listdir(sources) :
      ...
      for fle in filenames:
         ...
         if f[len(f)-1].lower() in ['.cmdf']:   # /!\ you found at least one cmdf file
            if rescan:
               if not moddir in tobedel: tobedel.update({moddir:[path.join(dirpath,fle)]}) # remove(path.join(dirpath,fle))
               else: tobedel[moddir].append(path.join(dirpath,fle))
            else:
      ...
      for subdir in dirnames:
         ...
         for subpath, subnames, filenames in walk(path.join(modroot,subdir)):
            if moddir not in ['utils']:
               for fle in filenames :
                  f = path.splitext(fle)
                  if f[len(f)-1].lower() in ['.f','.f90','.c']:       # /!\ you found a source file
                     if not moddir in modules: modules.update({moddir:{'path':modroot,'files':[path.join(subpath,fle).replace(modroot,'')[1:]],'cmdfs':[]}})
                     elif not 'files' in modules[moddir]: modules[moddir].update({'files':[path.join(subpath,fle).replace(modroot,'')[1:]]})
                     else: modules[moddir]['files'].append(path.join(subpath,fle).replace(modroot,'')[1:])
            else:
               for fle in filenames:
                  f = path.splitext(fle)
                  if f[len(f)-1].lower() in ['.dico']:   # /!\ you found an executable module
                  ...
                  if f[len(f)-1].lower() in ['.cmdf']:   # /!\ you found at least one cmdf file
                     if rescan:
                        if not subdir in tobedel: tobedel.update({subdir:[path.join(subpath,fle)]}) #remove(path.join(subpath,fle))
                        else: tobedel[subdir].append(path.join(subpath,fle))
                     else:

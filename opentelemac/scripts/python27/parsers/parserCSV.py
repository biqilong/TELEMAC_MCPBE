...
"""
"""@history 07/03/2018 -- Sebastien E. Bourban
         Additional support for PRN files (space-delimited)
"""
...
def cleanSpaces(istr): # same as in parserFortran
   istr = istr.strip()
   while '  ' in istr: istr = istr.replace('  ',' ')
   while ' ,' in istr: istr = istr.replace(' ,',',')
   while ', ' in istr: istr = istr.replace(', ',',')
   return istr
...
class CSV:
   ...
   def getFileContent(self,fileName):
      if not path.exists(fileName):
         print( '... could not find your ASCII file: '+fileName )
         sys.exit(1)
      ...
      vars = cleanSpaces(line).split(self.char)
      self.colvars = vars[0]
      self.rowvars = vars[1:]
      # ~~> column header - unit
      line = SrcF.readline()
      units = cleanSpaces(line).split(self.char)
      self.colunits = units[0]
      self.rowunits = units[1:]
      if len(self.rowvars) != len(self.rowunits):
         print( '... variables and units are incorrectly numbered in '+fileName )
         sys.exit(1)
      # ~~> parse main values
      #SrcF.seek(0)
      #data = np.loadtxt(SrcF, comments='#', skiprows=4, delimiter=self.char)
      data = np.loadtxt(SrcF, comments='#', delimiter=self.char)
      self.colcore = data.T
      # ~~> closure
      SrcF.close()

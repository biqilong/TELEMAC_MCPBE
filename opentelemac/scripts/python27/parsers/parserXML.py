r"""@author David H. Roscoe and Sebastien E. Bourban
...
from parserKeywords import scanDICO, scanCAS, readCAS, rewriteCAS, \
                           translateCAS, getCASLang, getKeyWord, \
                           setKeyValue, getIOFilesSubmit
...
from utils.files import getFileContent, putFileContent, addFileContent, createDirectories, copyFile, moveFile, \
   matchSafe, diffTextFiles
...
#
def mapdiff(a1, a2, notime=False, noname=False, relative=False):
   """
   ...
   if relative:
       for ivar in range(nvar):
           for i in range(npoin):
               ldiff = a2.values[0][ivar][i] \
                          - a1.values[0][ivar][i]
               if abs(ldiff) > 1e-42:
                   a_max = max(abs(a2.values[0][ivar][i]), \
                               abs(a1.values[0][ivar][i]))
...
def norm_l2(diff):
    ...
    @param diff Vector containing (x1 - x2)
    """
    err = math.sqrt(sum(abs(diff*diff)))/len(diff)
    return err
...
def checkval(a0, eps, norm='linf'):
   ...
   for ivar in range(nvar):
      if norm == 'l1':
         err = norm_l1(a0.values[0][ivar])
      elif norm == 'l2':
         err = norm_l2(a0.values[0][ivar])
      elif norm == 'linf':
         err = norm_linf(a0.values[0][ivar])
      else:
         err = norm_linf(a0.values[0][ivar])
...
class ACTION:
   ...
   def addAction(self, actions, rank=''):
      ...
      if self.active["xref"] in self.dids:
         raise Exception([{'name': 'ACTION::addACTION',
                       'msg': 'you are getting me confused, this xref already exists: ' + self.active["xref"]}])
      ...
      found = False
      try:
         found = isinstance(self.active["deprefs"], basestring)
      except:
      ...
      found = False
      try:
         found = isinstance(self.active["outrefs"], basestring)
      except:
   ...
   def updateCFG(self, d):
      self.dids[self.active["xref"]][self.active['cfg']].update(d)
...
class GROUPS:
   ...
   def addGroup(self, group, rank=''):
      ...
      if self.tasks["xref"] in self.dids[self.active['type']]:
         raise Exception([{'name': 'GROUP::addGroup',
                       'msg': 'you are getting me confused, this xref already exists: ' + self.tasks["xref"]}])
...
class actionRUN(ACTION):
   ...
   def translateCAS(self, rebuild):
      ...
      for mod in active["links"]:
         ...
         if matchSafe(casFile, link['target'] + '.??', oneup, rebuild):
            ...
            casfr, casgb = translateCAS(casFile,
                                 DICOS[link['dico']]['frgb'])  # /!\ removes comments at end of lines
            moveFile(casfr, oneup)
            moveFile(casgb, oneup)
            updated = True
      return updated
               ...
               if layer != []:
                  if path.isfile(layer[0][0]):
                     try:
                        ...
                        raise Exception([filterMessage({'name': 'runCommand',
                                                'msg': 'I can see your input file ' + layer[0][
                                                   0] + 'but cannot copy it'}, e, True)])
                  else:
                     raise Exception([{'name': 'runCommand',
                                   'msg': 'could not find reference to the dependant: ' + layer[0][0]}])
            else:
               raise Exception([{'name': 'runCommand', 'msg': 'could not find the configuration ' + self.active[
                  'cfg'] + 'for the dependant: ' + oFile}])
         else:
            if self.active["where"] != '':
               if path.exists(path.join(self.active["where"], oFile)): copyFile(
                  path.join(self.active["where"], oFile), self.active['safe'])
            elif path.exists(path.join(self.path, oFile)):
               copyFile(path.join(self.path, oFile), self.active['safe'])
            else:
               raise Exception(
                  [{'name': 'runCommand', 'msg': 'could not find reference to the dependant: ' + oFile}])
            ...
            if cfgname in self.dids[xref]:
               ...
               if self.active["deprefs"][xref].lower() == "sortie":
                  if path.isfile(active["sortie"]):
                     copyFile(active["sortie"], self.active['safe'])
                  else:
                     raise Exception([{'name': 'runCommand',
                                   'msg': 'could not find reference to the sortie: ' + active["sortie"]}])
                  ...
                  if path.exists(path.join(active['safe'], active["outrefs"][self.active["deprefs"][xref]])):
                     try:
                        copyFile(path.join(active['safe'], active["outrefs"][self.active["deprefs"][xref]]),
                               self.active['safe'])
                     except Exception as e:
                        raise Exception([filterMessage({'name': 'runCommand',
                                                'msg': 'I can see your file ' + self.active["deprefs"][
                                                   xref] + 'but cannot copy it'}, e, True)])
                  else:
                     raise Exception([{'name': 'runCommand',
                                   'msg': 'I cannot see your output file ' + self.active["deprefs"][xref]}])
               else:
                  layer = []
                  for oFile in active["output"]:
                     ...
                     if layer != []:
                        if path.isfile(layer[0][0]):
                           ...
                           raise Exception([{'name': 'runCommand',
                                         'msg': 'could not find reference to the dependant: ' + layer[0][
                                            0]}])
                  for mod in active["links"]:
                     for iFile in active["links"][mod]["oFS"]:
                        layer = findTargets({'code': mod, 'output': active["links"][mod]["oFS"]},
                        ...
                        if layer != []:
                           if path.isfile(layer[0][0]):
                              try:
                                 ...
                                 raise Exception([filterMessage({'name': 'runCommand',
                                                         'msg': 'I can see your input file ' +
                                                               layer[0][0] + 'but cannot copy it'},
                                                         e, True)])
                           else:
                              raise Exception([{'name': 'runCommand',
                                            'msg': 'could not find reference to the dependant: ' +
                                                 layer[0][0]}])
                  if layer == []: raise Exception([{'name': 'runCommand',
                                            'msg': 'could not find reference to the linked file: ' +
                                                 self.active["deprefs"][xref]}])
         ...
         if path.exists(path.join(self.active['safe'], oFile)):
            try:
               ...
               raise Exception([filterMessage(
                  {'name': 'runCommand', 'msg': 'I can see your file ' + oFile + ' but cannot copy it'}, e,
                  True)])
         else:
            raise Exception([{'name': 'runCommand',
                          'msg': 'I cannot see your output file ' + oFile + ' from within action reference ' +
                               self.active["xref"]}])
   ...
   def diffPRINCI(self, options, cfg, rebuild):
     ...
     """
      ...
      value, default = getKeyWord('FICHIER FORTRAN', active['cas'], DICOS[active['dico']]['dico'],
            ...
            if matchSafe(htmlFile, path.basename(htmlFile), oneup, rebuild):
               ...
               if pFiles == []:
                  raise Exception([{'name': 'ACTION::diffPRINCI',
                                'msg': 'I could not recognised entities in your PRINCI: ' + princiFile}])
               else:
                  print('        +> found:')
               ...
               if oFiles == {}:
                  print(
                  '/!\\ warning: could not relate the following in your PRINCI with the system: ' + princiFile)
   ...
   def compilePRINCI(self, cfg, rebuild):
      ...
      try:
         ...
         plib = cfg['MODULES'][active["code"]]['path'].replace(cfg['root'] + sep + 'sources', cfg[
            'root'] + sep + 'builds' + sep + cfgname + sep + 'lib')
         exeFile, updated = \
   ...
   def runCommand(self, rebuild):
      ...
      if path.isfile(path.join(self.active['path'], self.active["target"])):
         ...
         try:
            ...
            raise Exception([filterMessage(
               {'name': 'runCommand', 'msg': 'I can see your input file ' + iFile + 'but cannot copy it'}, e,
               True)])
      else:
         ...
         if iFile == '': raise Exception(
            [{'name': 'runCommand', 'msg': 'could not find reference to the target: ' + iFile}])
      ...
      try:
         ...
         raise Exception([filterMessage(
            {'name': 'runCommand', 'msg': 'something went wrong when executing you command.'}, e, True)])
      if code != 0: raise Exception(
         [{'name': 'runCommand', 'msg': 'Could not run your command (' + exeCmd + ').\n      ' + tail}])
      ...
      for oFile in self.active["outrefs"]:
         if path.exists(path.join(self.active['safe'], self.active["outrefs"][oFile])):
            try:
               ...
               raise Exception([filterMessage({'name': 'runCommand', 'msg': 'I can see your file ' + oFile + ': ' +
                                                             self.active["outrefs"][oFile] + 'but cannot copy it'}, e, True)])
         else:
            raise Exception([{'name': 'runCommand',
                          'msg': 'I cannot see your output file ' + oFile + ': ' + self.active["outrefs"][
                             oFile]}])
      return updated
...
class groupCAST(GROUPS):
   ...
   def addCast(self, cast, rank=''):
      ...
      if self.dids[self.active['type']][self.tasks["xref"]]['rank'] == '':
         self.dids[self.active['type']][self.tasks["xref"]]['rank'] = '953'
      self.dids[self.active['type']][self.tasks["xref"]]['rank'] = int(
...
def runXML(xmlFile, xmlConfig, reports, bypass, runOnly):
   ...
   rankdo = int(rank)
   rankdont = 0
   if xmlConfig[xmlConfig.keys()[0]]['options'].rank != '': rankdont = int(
      xmlConfig[xmlConfig.keys()[0]]['options'].rank)
   ...
   for decoing in xmlRoot.findall("deco"):
      ...
      if len(decoing.findall("look")) > 1:
         raise Exception(
            [{'name': 'runXML', 'msg': 'you can only have one look in deco referenced: ' + dc.tasks["xref"]}])
      ...
      if len(decoing.findall("data")) > 1:
         raise Exception(
            [{'name': 'runXML', 'msg': 'you can only have one data in deco referenced: ' + dc.tasks["xref"]}])
   ...
   if xcpt != []:
      raise Exception({'name': 'runXML', 'msg': 'looking at deco in xmlFile: ' + xmlFile, 'tree': xcpt})
   ...
   for xmlChild in xmlRoot:
      ...
      if xmlChild.tag == "action":
         ...
         try:
            ...
            report.update(
               {'xref': do.active['xref'], 'updt': False, 'fail': False, 'warn': False, 'rank': do.active['rank'],
                'value': 0, 'title': 'This was ignored'})
         except Exception as e:
         ...
         for cfgname in xmlConfig:
            ...
            if "config" in do.active.keys() and not oneFound:
               ...
               oneFound = True
         if not oneFound:
            raise Exception([{'name': 'runXML',
                          'msg': 'could not find the specified configuration made up of the following string(s) ' +
                               do.active["config"] + ' on your system.\n      ... do check action ' +
                               do.active['xref'] + ' within ' + path.basename(xmlFile)}])
         ...
         for cfgname in xmlConfig:
            ...
            if do.active["code"] == 'mascaret':
               ...
               if do.active["ncsize"] != '': cas = setKeyValue('PROCESSEURS PARALLELES', cas, frgb,
                                                   int(do.active["ncsize"]))
               ...
               sortieFiles, iFS, oFS = setSafe(casFile, cas, idico, odico,
                                       do.active['safe'])  # TODO: look at relative paths
               ...
               for cplage in cplages:
                  for mod in cfg['MODULES']:
                     if mod in cplage.lower():
                        ...
                        if casFilePlage == []:
                           casFilePlage = defaut[0]
                        else:
                           casFilePlage = casFilePlage[0].strip("'\"")
                        casFilePlage = path.join(path.dirname(casFile), casFilePlage)
                        if not path.isfile(casFilePlage):
                           raise Exception([{'name': 'runCAS',
                                         'msg': 'missing coupling CAS file for {}: {}'.format(mod, casFilePlage)}])
                        # ~~> Read the DICO File
                        ...
                        sortiePlage, iFSPlage, oFSPlage = setSafe(casFilePlage, casPlage, idicoPlage,
                                                        odicoPlage, do.active['safe'])  # TODO: look at relative paths
                        links.update({mod: {}})
                        ...
                        if sortiePlage != []:
                           links[mod].update({'sortie': sortiePlage})
               if links != {}:
                  do.updateCFG({"links": links})
            ...
            # ~~ Step 3b. Deals with execute launchers ~~~~~~~~~~~~~
               ...
               if "exec" in doable.split(';'):
                  try:
                     ...
                     xcpt.append(
                        filterMessage({'name': 'runXML::runCommand', 'msg': '   +> ' + do.active["do"]}, e,
                                   bypass))
         ...
         if xcpt != []: raise Exception(
            {'name': 'runXML', 'msg': 'looking at actions in xmlFile: ' + xmlFile, 'tree': xcpt})
      ...
      if xmlChild.tag[0:4] == "cast" and not runOnly:
         ...
         if len(casting.findall("python")) > 1:
            raise Exception([{'name': 'runXML',
                          'msg': 'you can only have one python key in cast referenced: ' + cast.tasks["xref"]}])
         ...
         if len(casting.findall("return")) > 1:
            raise Exception([{'name': 'runXML',
                          'msg': 'you can only have one return key in cast referenced: ' + cast.tasks["xref"]}])
         ...
         for vari in casting.getchildren():
            ...
            if vari.tag in cast.tasks:
               raise Exception([{'name': 'runXML',
                             'msg': 'The variable {} is cast more than once in {}'.format(vari.tag, cast.tasks["xref"])}])
            try:
            ...
            if xdid != '':
               if src not in caster.obdata.keys(): raise Exception(
                  [{'name': 'runXML', 'msg': 'did not already cast variable ' + src + ' wihtin ' + xref}])
               layers = {}
               for cfgname in xmlConfig:
            ...
            if xdid != '':  # saved files can be reopened as target to do something else
               ...
               if save.dids[xdid][xref]["where"] != '':
                  if path.exists(path.join(save.dids[xdid][xref]["where"], target)):
                     ...
                     for cfgname in xmlConfig:
                        findlayer.update({cfgname: [[path.join(save.dids[xdid][xref]["where"], target)], '', t]})
                     cast.targetSubTask(findlayer, index, namex)
                  else:
                     raise Exception([{'name': 'runXML',
                                   'msg': 'could not find reference to extract the cast: ' + target + ' where ' +
                                        cast.tasks[namex][index]["where"]}])
               elif path.exists(path.join(save.path, target)):
                  findlayer = {}
                  t = path.splitext(path.basename(target))[1].lower()[1:]
                  for cfgname in xmlConfig:
                     findlayer.update({cfgname: [[path.join(save.path, target)], '', t]})
                  cast.targetSubTask(findlayer, index, namex)
               continue  # try finding the next variable
            ...
            if xref in do.dids:
               ...
               if oneFound and layers == {}:
                  raise Exception([{'name': 'runXML',
                                'msg': 'could not find reference to cast the action: {}:{}'.format(xref, src)}])
               else:
            ...
            if src == '':
               if cast.tasks["where"] != '':
                  if path.exists(path.join(cast.tasks["where"], target)):
                     ...
                     t = path.splitext(path.basename(target))[1].lower()[1:]
                     for cfgname in xmlConfig:
                        findlayer.update({cfgname: [[path.join(cast.tasks["where"], target)], '', t]})
                     cast.targetSubTask(findlayer, index, namex)
                  else:
                     raise Exception([{'name': 'runXML',
                                   'msg': 'could not find reference to extract the cast: ' + target + ' where ' +
                  ...
                  raise Exception([{'name': 'runXML', 'msg': 'could not find reference to cast: ' + target}])

         cast.update(cast.tasks)

         if xcpt != []: raise Exception(
            {'name': 'runXML', 'msg': 'looking at casting in xmlFile: ' + xmlFile, 'tree': xcpt})
         ...
         for var in task["vars"]:
            if task['config'] == 'together':
               raise Exception([{'name': 'runXML',
                             'msg': 'could not have more than one configuration at a time for casting: ' + var[
                                'xref']}])
         ...
         if 'return' not in cast.tasks:
            ...
            for var in cast.tasks['return']:
               if var in ['fail', 'warn', 'value']:
                  ...
                  report.update({var: r})
                  print('        - cast: ' + var + ' = ' + repr(r) + ' ( expression: ' + cast.tasks['return'][
                     var] + ' )')
                  # if epsilon was reached then we crash
                  if var in ['fail'] and r:
                     raise Exception([{'name':'runXML',
                                       'msg':'Epsilon was reached for the test case with "'\
                                             + cast.tasks['return'][var] + '"'}])  # only one item here
               else:
      ...
      if xmlChild.tag[0:4] == "save" and not runOnly:
         ...
         if "type" not in xmlChild.attrib:  # TODO: This will eventually be removed
            if len(typeSave) == 4:
               raise Exception([{'name': 'runXML',
                             'msg': 'do not know what dimension your saved data will be in: ' +
                                  xmlChild.attrib["xref"]}])
               ...
               raise Exception([{'name': 'runXML',
                             'msg': 'the type of your saved data should be in: ["save1d","save2d","save3d"] for ' +
                                  xmlChild.attrib["xref"]}])
         else:
            if len(typeSave) == 4:
               if len(xmlChild.attrib["type"].split(':')) == 2:
                  typeSave = typeSave + xmlChild.attrib["type"].split(':')[0]
               else:
                  raise Exception([{'name': 'runXML',
                                'msg': 'do not know what dimension your saved data will be in: ' +
                                     xmlChild.attrib["xref"]}])
               ...
               if len(xmlChild.attrib["type"].split(':')) == 2:
                  if typeSave[4:6] != xmlChild.attrib["type"].split(':')[0]: raise Exception([{'name': 'runXML',
                                                                            'msg': 'inconsistency in the dimension of your plotted data in: ' +
                                                                                 xmlChild.attrib[
                                                                                    "xref"]}])
               else:
                  xmlChild.attrib["type"] = typeSave[4:6] + ":" + xmlChild.attrib["type"]
            else:
               raise Exception([{'name': 'runXML',
                             'msg': 'the type of your saved data should be in: ["save1d","save2d","save3d"] for ' +
                                  xmlChild.attrib["xref"]}])
         ...
         if save.dids[typeSave][save.active['xref']]['saveas'] == '':
            if typeSave[4:6] == "1d":
               save.dids[typeSave][save.active['xref']]['saveas'] = 'csv'
            if typeSave[4:6] == "2d":
               save.dids[typeSave][save.active['xref']]['saveas'] = 'slf'
            if typeSave[4:6] == "3d":
               save.dids[typeSave][save.active['xref']]['saveas'] = 'slf'
         ...
         for layer in extracting.findall("layer"):
            ...
            if xref in cast.dids['cast']:
               ...
               if layers == {}:
                  raise Exception([{'name': 'runXML',
                                'msg': 'could not find reference to save the cast: ' + xref + ':' + src}])
               ...
               if xref in do.dids:
                  ...
                  if oneFound and layers == {}:
                     raise Exception([{'name': 'runXML',
                                   'msg': 'could not find reference to extract within actions: ' + xref + ':' + src}])
                     save.targetSubTask({}, index, namex)
                     continue  # bypass the rest of the for loop
                  else:
                     save.targetSubTask(layers, index, namex)
                  ...
                  raise Exception([{'name': 'runXML',
                                'msg': 'could not find reference to extract the action: ' + xref + ':' + src}])
               ...
               if save.tasks[namex][index]["where"] != '':
                  if path.exists(path.join(save.tasks[namex][index]["where"], target)):
                     findlayer = {}
                     t = path.splitext(path.basename(target))[1].lower()[1:]
                     for cfgname in xmlConfig: findlayer.update(
                        {cfgname: [[path.join(save.tasks[namex][index]["where"], target)], '', t]})
                     save.targetSubTask(findlayer, index, namex)
                  else:
                     raise Exception([{'name': 'runXML',
                                   'msg': 'could not find reference to extract the action: ' + target + ' where ' +
                  ...
                  raise Exception(
                     [{'name': 'runXML', 'msg': 'could not find reference to extract the action: ' + target}])
               ...
               if save.tasks['deco'] != '':
                  ...
                  for deco in decos:
                     if deco in dc.dids['deco']:
                        ...
                        raise Exception([{'name': 'runXML',
                                      'msg': 'could not find reference to deco tag <' + deco + '> for figure "' +
                                           save.tasks['xref'] + '"'}])
               elif save.tasks['xref'] in dc.dids['deco']:
                  save.tasks['deco'] = dc.dids['deco'][save.tasks['xref']]
               else:
                  if typeSave[4:6] == '1d':
                     save.tasks['deco'] = decoDefault1D
                  if typeSave[4:6] == '2d':
                     save.tasks['deco'] = decoDefault2D
                  if typeSave[4:6] == '3d':
                     if not MAYAVI_AVAIL:
                        return
                     save.tasks['deco'] = decoDefault3D
            # > at the layer level
            if type(save.tasks[namex][index]['deco']) == type(''):
               if save.tasks[namex][index]['deco'] != '':
                     ...
                     if deco in dc.dids['deco']:
                        for name in dc.dids['deco'][deco]:
                           if name in ['look', 'data']:
                              save.tasks[namex][index]['deco'].update(dc.dids['deco'][deco][name][0])
                     else:
                        raise Exception([{'name': 'runXML',
                                      'msg': 'could not find reference to deco tag <' + deco + '> for figure "' +
                                           save.tasks['xref'] + '"'}])
                  ...
                  if typeSave[4:6] == '1d':
                     save.tasks[namex][index]['deco'] = decoDefault1D
                  if typeSave[4:6] == '2d':
                     save.tasks[namex][index]['deco'] = decoDefault2D
                  if typeSave[4:6] == '3d':
                     save.tasks[namex][index]['deco'] = decoDefault3D

         save.update(save.tasks)

         if xcpt != []:
            raise Exception({'name': 'runXML', 'msg': 'looking at extractions in xmlFile: ' + xmlFile, 'tree': xcpt})
         ...
         for layer in task["layers"]:
            if layer['config'] == 'together':
               ...
               for x in xlayers.split('|'):
                  xys.append((x + ';' + ':'.join(layer['fileName'].keys())).strip(';'))
               xlayers = '|'.join(xys)
               ...
               xys = [];
               cfg = layer['fileName'].iterkeys().next()  # /!\ you are sure to have at least one (?)
               for x in xlayers.split('|'):
            ...
            for layer, cfgs in zip(task["layers"], cfglist.split(';')):
               for cfg in cfgs.split(':'):
                  for fle in layer['fileName'][cfg][0]:
                     figure.add(layer['fileName'][cfg][2],
                              {'file': fle,
                              'deco': {}, 'xref': xref,
                              'vars': layer["vars"], 'extract': layer["extract"],
                              'sample': layer["sample"],
         ...
         if xcpt != []: raise Exception(
            {'name': 'runXML', 'msg': 'looking at savings in xmlFile: ' + xmlFile, 'tree': xcpt})
      ...
      if xmlChild.tag[0:4] == "plot" and not runOnly:
         ...
         if "type" not in xmlChild.attrib:  # TODO: This will eventually be removed
            if len(typePlot) == 4:
               raise Exception([{'name': 'runXML',
                             'msg': 'do not know what dimension your plotted data will be in: ' +
                                  xmlChild.attrib["xref"]}])
            elif len(typePlot) == 6:  # defaults for each type
               if typePlot[4:6] == '1d':
                  xmlChild.set("type", typePlot[4:6] + ':history')
               if typePlot[4:6] == '2d':
                  xmlChild.set("type", typePlot[4:6] + ':p-section')
               if typePlot[4:6] == '3d':
                  xmlChild.set("type", typePlot[4:6] + ':i-surface')
            else:
               raise Exception([{'name': 'runXML',
                             'msg': 'the type of your plot should be in: ["plot1d","plot2d","plot3d","plotpv"] for ' +
                                  xmlChild.attrib["xref"]}])
         else:
            if len(typePlot) == 4:
               if len(xmlChild.attrib["type"].split(':')) == 2:
                  typePlot = typePlot + xmlChild.attrib["type"].split(':')[0]
               else:
                  raise Exception([{'name': 'runXML',
                                'msg': 'do not know what dimension your plotted data will be in: ' +
                                     xmlChild.attrib["xref"]}])
               ...
               if len(xmlChild.attrib["type"].split(':')) == 2:
                  if typePlot[4:6] != xmlChild.attrib["type"].split(':')[0]: raise Exception([{'name': 'runXML',
                                                                            'msg': 'inconsistency in the dimension of your plotted data in: ' +
                                                                                 xmlChild.attrib[
                                                                                    "xref"]}])
               else:
                  xmlChild.attrib["type"] = typePlot[4:6] + ":" + xmlChild.attrib["type"]
            else:
               raise Exception([{'name': 'runXML',
                             'msg': 'the type of your plot should be in: ["plot1d","plot2d","plot3d","plotpv"] for ' +
                                  xmlChild.attrib["xref"]}])
         ...
         for layer in ploting.findall("layer"):
            ...
            if xref in cast.dids['cast']:
               ...
               if layers == {}:
                  raise Exception([{'name': 'runXML',
                                'msg': 'could not find reference to draw the cast: ' + xref + ':' + src}])
               ...
               if xref in save.dids:
                  ...
                  if oneFound and layers == {}:
                     raise Exception([{'name': 'runXML',
                                   'msg': 'could not find reference to draw the extract: ' + xref + ':' + src}])
                     plot.targetSubTask({}, index, namex)
                     continue  # bypass the rest of the for loop
                  else:
                     plot.targetSubTask(layers, index, namex)
                  ...
                  for cfgname in xmlConfig:
                     ...
                     if findlayer != []: layers.update({cfgname: findlayer})
                  if oneFound and layers == {}:
                     xcpt.append({'name': 'runXML',
                               'msg': 'could not find reference to draw the action: ' + xref + ':' + src})
                     plot.targetSubTask({}, index, namex)
                     continue  # bypass the rest of the for loop
                  else:
                     plot.targetSubTask(layers, index, namex)
               ...
               if plot.tasks[namex][index]["where"] != '':
                  if path.exists(path.join(plot.tasks[namex][index]["where"], target)):
                     ...
                     t = path.splitext(path.basename(target))[1].lower()[1:]
                     for cfgname in xmlConfig: findlayer.update(
                        {cfgname: [[path.join(plot.tasks[namex][index]["where"], target)], '', t]})
            ...
            if type(plot.tasks['deco']) == type(''):
               if plot.tasks['deco'] != '':
                  ...
                  if typePlot[4:6] == '1d':
                     plot.tasks['deco'] = {}  # {'look':[decoDefault1D]}
                  if typePlot[4:6] == '2d':
                     plot.tasks['deco'] = decoDefault2D
                  if typePlot[4:6] == '3d':
                     if not MAYAVI_AVAIL:
                        return
                     plot.tasks['deco'] = decoDefault3D
            # > at the layer level
               ...
               if plot.tasks[namex][index]['deco'] != '':
                  ...
                  for deco in decos:
                     if deco in dc.dids['deco']:
                        for name in dc.dids['deco'][deco]:
                           if name in ['look', 'data']:
                              plot.tasks[namex][index]['deco'].update(dc.dids['deco'][deco][name][0])
                     else:
                        raise Exception([{'name': 'runXML',
                                      'msg': 'could not find reference to layer deco tag: ' + deco + ' for figure ' +
                                           plot.tasks['xref']}])
               else:
                  if typePlot[4:6] == '1d':
                     plot.tasks[namex][index]['deco'] = {}  # {'look':[decoDefault1D]}
                  if typePlot[4:6] == '2d':
                     plot.tasks[namex][index]['deco'] = decoDefault2D
                  if typePlot[4:6] == '3d':
                     if not MAYAVI_AVAIL:
                        return
                     plot.tasks[namex][index]['deco'] = decoDefault3D

         plot.update(plot.tasks)

         if xcpt != []: raise Exception(
            {'name': 'runXML', 'msg': 'looking at targets in xmlFile: ' + xmlFile, 'tree': xcpt})
         ...
         for cfglist in alayers:
            ...
            elif typePlot[4:6] == "2d":
               figure = Figure2D(caster, draw)
            elif typePlot[4:6] == "3d":
               if not MAYAVI_AVAIL:
                  return
         ...
         if xcpt != []: raise Exception(
            {'name': 'runXML', 'msg': 'looking at plotting in xmlFile: ' + xmlFile, 'tree': xcpt})

...
class VnvStudy(AbstractVnvStudy):
    ...
    def _pre(self):
        ...
        #======================================================================
        # waf run
        self.add_study('waf_seq', 'telemac2d', 't2d_thacker-waf.cas')

        #======================================================================
        # kin1 run
        self.add_study('kin1_seq', 'telemac2d', 't2d_thacker-kin1.cas')
        # kin1 run in parallel
        cas = TelemacCas('t2d_thacker-kin1.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)
        self.add_study('kin1_par', 'telemac2d', 't2d_thacker-kin1_par.cas', cas=cas)
        del cas

        #======================================================================
        # hllc run
        self.add_study('hllc_seq', 'telemac2d', 't2d_thacker-hllc.cas')
        # hllc run in parallel
        cas = TelemacCas('t2d_thacker-hllc.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)
        self.add_study('hllc_par', 'telemac2d', 't2d_thacker-hllc_par.cas', cas=cas)
        del cas
    ...
    def _check_results(self):
        ...
        self.check_epsilons('nerd_seq:T2DRES', 'f2d_thacker.slf',
                            eps=[1e-4, 1e-4, 1e-4, 1e-4, 1.E-15,\
                                 1.E-15, 1.E-15, 1.E-15, 1.E-15])

        # sequential parallel comparison
        self.check_epsilons('char_seq:T2DRES', 'char_par:T2DRES', eps=[2e-1])
        self.check_epsilons('nerd_seq:T2DRES', 'nerd_par:T2DRES', eps=[2e-1])

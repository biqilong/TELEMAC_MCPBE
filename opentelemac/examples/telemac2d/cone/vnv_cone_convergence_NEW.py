...
class VnvStudy(AbstractVnvStudy):
    ...
    def _init(self):
        ...
        # Time discretization:
        # WARNING: variable timestep not compatible with FE methods atm.
        self.variable_timestep = False
        self.CFL = 0.9
        self.timestep = self.time_period/64.

        # Numerical schemes tested:
        self.schemes = [
            'SCHAR', 'N', 'N PC1', 'N PC2', 'N LIPS', 'ERIA',
            'WCHAR', 'PSI', 'PSI PC1', 'PSI PC2', 'PSI LIPS', 'NERD']
        self.scheme_adv = [1, 4, 4, 4, 4, 15, 1, 5, 5, 5, 5, 14]
        self.scheme_opt = [1, 1, 2, 3, 4, 1 , 2, 1, 2, 3, 4, 1]
    ...
    def _pre(self):
        ...
        for ids, sc in enumerate(self.schemes):
            ...
            # set base study:
            cas = TelemacCas('t2d_cone_convergence.cas', get_dico('telemac2d'))
            ...
            # set scheme options:
            cas.set('SCHEME FOR ADVECTION OF TRACERS', self.scheme_adv[ids])
            cas.set('SCHEME OPTION FOR ADVECTION OF TRACERS', self.scheme_opt[ids])
            ...
            self.add_study('{}_mesh0'.format(sc), 'telemac2d',\
                           't2d_cone-{}.cas'.format(sc), cas=cas)
            ...
            for i in range(self.refinement_levels):
                ...
                self.add_study('{}_mesh{}'.format(sc, i+1), 'telemac2d',
                               't2d_cone-{}_{}.cas'.format(sc, i+1), cas=cas)
                ...
                for j, res in enumerate(res_list):
                    ...
                    if res_labels[j].split('_')[0] == sc:
                        ...
                        for ite in range(res.ntimestep):
                            ...
                            # Compute diff
                            data = res.get_data_value('TRACER', ite)
...
#                    # Compute diff
#                    data = res.get_data_value('TRACER', -1) 

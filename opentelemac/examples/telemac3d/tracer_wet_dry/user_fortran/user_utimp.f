...
!
     ...
     &          DT*FLUXTOTCUM,' S = ',MASSE_WATER,' M3'
      ...
      IF(NCSIZE.GT.1) THEN
        IF(IPID.EQ.0) THEN
          IF(LT.EQ.1) THEN
            OPEN(UNIT=1846986,FILE='../tracer.txt')
            WRITE(1846986,*) 'MASS'
            MASS_T_0 = MASSE%R(6) ! NOT MASINI%R(6)=0...
            WRITE(1846986,1003) 'TIME',INT(0),'  CREATED ',
     &            DT*FLUCUM%R(6),' S = ',MASS_T_0,' M3'
            CLOSE(1846986)
          ELSEIF(MOD(LT,GRAPRD).EQ.0) THEN
            OPEN(UNIT=1846986,FILE='../tracer.txt',
     &           POSITION='append')
            WRITE(1846986,1003) 'TIME',INT(AT),'  CREATED ',
     ...
     &           POSITION='append')
            ...
            WRITE(1846986,*) "MASS LOST = ", PERDUE_T

...
beginning of a computation using the \telfile{USER\_T3D\_CORFON} subroutine. This is
...
\telfile{USER\_T3D\_CORFON} subroutine) carries out a number of bottom
...
The modification is done in the \telfile{USER\_CORRXY} subroutine (\bief library),
...
3D PRINTOUTS}. Sequences of 8 letters may be used.
...
\telfile{CORSTR}, \telfile{USER\_BORD3D}, etc.  If a \telfile{PRIVE} table is used
...
computation, in the form of a subroutine named \telfile{BIEF\_VALIDA}, which has to
...
The \telfile{BIEF\_VALIDA} subroutine is called at each time step when the keyword
\telkey{VALIDATION} has the value YES, enabling a comparison to be made with
the validation solution at each time step. By default, the \telfile{BIEF\_VALIDA}
...
in the \telemac{3d} steering file.

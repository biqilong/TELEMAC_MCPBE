...
\section{Culverts}
\label{sec:culverts}
The keyword \telkey{NUMBER OF CULVERTS} specifies the number of culverts to be treated
as source terms.
...
Information about culvert characteristics is stored in the \telkey{CULVERTS DATA FILE}.
...
computed by \telemac{3d}
or whether the data file values are used to set these angles: AA=1 -- automatic angle; AA=0 -- user-set angle.\\

There are two options in \telemac{3d} to treat culverts.
It can be chosen with the \telkey{OPTION FOR CULVERTS} keyword

...
It is a table that contains up to \telfile{MAXFRO}
...
the \telkey{BOUNDARY CONDITIONS FILE}.
\end{itemize}
...
flowrate.
It is a table that contains up to \telfile{MAXFRO} real numbers for managing up
...
\telfile{Q} for flow rate, \telfile{SL} for water level, \telfile{U} and
\telfile{V} for velocities and \telfile{TR} for tracer.
...
\telkey{PRESCRIBED {\dots}} to trigger the use of the liquid boundary file.
\end{WarningBlock}
...
using the keyword \telkey{STAGE-DISCHARGE CURVES} which supply one integer per
liquid boundary.
This integer can be:
...
This is usual for coastal domains where only the values of the sea level on
several points are known.

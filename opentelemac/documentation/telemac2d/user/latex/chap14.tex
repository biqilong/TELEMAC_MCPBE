...
In the \telfile{USER\_NOMVAR\_TELEMAC2D} subroutine, it is possible
...
and \telfile{PRIVE\%ADR(4)\%P\%R(X)}, \telfile{PRIVE} (in which \telfile{X}
...
If a \telfile{PRIVE} table is used for programming a case, it is essential
...
The variables \telfile{LEO, SORLEO, IMP, SORIMP} are also used to determine
...
\telkey{COUPLING PERIOD FOR TOMAWAC} which default values are 1
...
This function is activated by the keyword \telkey{FOURIER ANALYSIS PERIODS} and

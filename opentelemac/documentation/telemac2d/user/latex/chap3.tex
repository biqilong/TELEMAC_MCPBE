...
Topographical and bathymetric data may be supplied to \telemac{2D} at three
levels:

\begin{enumerate}
\item Either directly in the \telkey{GEOMETRY FILE} by a topographical
...
BLUEKENUE, or when the \stbtel module is run before \telemac{2D} is started.
...
In contrast to \stbtel, \telemac{2D} only manages one bottom topography file.

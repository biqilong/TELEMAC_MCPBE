...
Option 6 can only be used in the \telkey{FRICTION DATA FILE}
...
with the keyword \telkey{NON-SUBMERGED VEGETATION FRICTION} (default = NO).

In this case, 2 additional data must be given:
\telkey{DIAMETER OF ROUGHNESS ELEMENTS} and
\telkey{SPACING OF ROUGHNESS ELEMENTS},
...
The calibration coefficients $\alpha_t$ and $C_l$ may be changed in the
\telfile{MIXLENGTH} subroutine.
Default values are $\alpha_t = \frac{0.4}{6}$ and $C_l = 0.2666667$.
...
\telkey{COEFFICIENT OF WIND INFLUENCE} (see {\S} below).
...
The parameter \telkey{COEFFICIENT OF WIND INFLUENCE} (default = 0.)
asked for by \telemac{2D} is:

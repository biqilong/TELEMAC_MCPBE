...
\telkey{INITIAL VALUES OF TRACERS} with the required value(s) in the steering
...
\telfile{USER\_CONDIN\_TRAC} subroutine, in a similar manner to that described
...
a particular tracer, \telemac{2D} then uses the value assigned to the keyword
...
The type of boundary condition will be given by the value of \telfile{LITBOR}
...
(\telfile{LITBOR} = 5), the value of the tracer may be given in various ways:
...
it will be directly set by the \telfile{TBOR} variable
...
of the keyword \telkey{TYPE OF ADVECTION}.

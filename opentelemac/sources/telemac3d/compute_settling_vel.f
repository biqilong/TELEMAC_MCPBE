...
!
     ...
     &    XWC0,TYPE_SED,MCPBE_VER,IMACFLC,IMEGFLC,IMICF_MACF,IMICF_MEGF
...
!
      ...
      ENDDO
!
!     GIVE SETTLING VELOCITY TO FLOC TRACERS      
      IF(FLOC .AND. FLOC_TYPE.EQ.3) THEN
        ...
        IF(MCPBE_VER.EQ.1) THEN
          DO IPOIN=1,NPOIN2
            DO IPLAN=1,NPLAN
              IPOIN3=(IPLAN-1)*NPOIN2+IPOIN
              WCHU%ADR(IMICF_MACF)%P%R(IPOIN3) = XWC0(IMACFLC)
            ENDDO
          ENDDO
        ENDIF
        ...
        IF(MCPBE_VER.GT.1) THEN
          DO IPOIN=1,NPOIN2
            DO IPLAN=1,NPLAN
              IPOIN3=(IPLAN-1)*NPOIN2+IPOIN
              WCHU%ADR(IMICF_MACF)%P%R(IPOIN3) = XWC0(IMACFLC)
              WCHU%ADR(IMICF_MEGF)%P%R(IPOIN3) = XWC0(IMEGFLC)
            ENDDO
          ENDDO
        ENDIF
...
! 3.FOR MCPBE FLOCCULATION MODEL
      IF(FLOC .AND. FLOC_TYPE.EQ.3) THEN
        IF(MCPBE_VER.EQ.1) CALL FLOC_WCHU_2CPBE
        IF(MCPBE_VER.GE.2) CALL FLOC_WCHU_3CPBE
      ENDIF
!
!-----------------------------------------------------------------------
!

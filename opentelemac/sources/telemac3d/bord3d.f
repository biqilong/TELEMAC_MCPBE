...
!
      ...
      INTEGER NPOINBND,NPOIN2BND,NPLANBND
      LOGICAL READ_BIN_Z,READ_BIN_U,READ_BIN_V,READ_BIN_TR,IERR
      LOGICAL YAZMIN,DEJA
...
!     -----------------------------------------------------------------
     ...
     &    READ_BIN_Z)
        CALL GET_MESH_NPOIN(T3D_FILES(T3DBND)%FMT,T3D_FILES(T3DBND)%LU,
     &                      QUADRANGLE_ELT_TYPE,NPOINBND,IERR)
        CALL GET_MESH_NPLAN(T3D_FILES(T3DBND)%FMT,T3D_FILES(T3DBND)%LU,
     &                      NPLANBND,IERR)
...
!
     ...
     &                    'IN THE BINARY FILE'
              DEJA = .TRUE.
            ENDIF
            HBOR%R(K) = T3_01%R(N+(NPLANBND-1)*NPOIN2BND)-ZF%R(IPOIN2)
          ELSE

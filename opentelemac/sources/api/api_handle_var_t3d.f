...
!
      MODULE API_HANDLE_VAR_T3D
        ...
        INTEGER, PARAMETER :: NB_VAR_T3D=107
...
!
     ...
     &  INST%TA%ADR(BLOCK_INDEX)%P%R(1:INST%TA%ADR(BLOCK_INDEX)%P%DIM1)
          ...
          END IF
        ELSE IF(TRIM(VARNAME).EQ.'MODEL.WIND') THEN
          IF(PRESENT(BLOCK_INDEX))THEN
          VALEUR(1:INST%WIND%ADR(BLOCK_INDEX)%P%DIM1) =
     &            INST%WIND%ADR(BLOCK_INDEX)%P%R
     &            (1:INST%WIND%ADR(BLOCK_INDEX)%P%DIM1)
          ELSE
             IERR = INDEX_BLOCK_MISSING
             ERR_MESS = 'THE BOCK NUMBER IS MISSING FOR'//TRIM(VARNAME)
          END IF
...
!
        ...
        ELSE IF(TRIM(VARNAME).EQ.'MODEL.WIND') THEN
          VALEUR = INST%WIND%ADR(INDEX1)%P%R(INDEX2)
        ELSE IF(TRIM(VARNAME).EQ.'WAQTEL.TAIR') THEN
...
!
        ...
        ELSE IF(TRIM(VARNAME).EQ.'MODEL.WIND') THEN
          INST%WIND%ADR(INDEX1)%P%R(INDEX2) = VALEUR
        ELSE IF(TRIM(VARNAME).EQ.'WAQTEL.TAIR') THEN
...
!
     ...
     &           'POINTS FOLLOWING AND PRECEDING A BOUNDARY POINT'
          ...
          VINFO_T3D(I) = 'NUMBER OF TRACER'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.MAXTRA'
          VINFO_T3D(I) = 'NUMBER OF MAXIMUM TRACER'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.MAXSCE'
          VINFO_T3D(I) = 'NUMBER OF MAXIMUM SOURCE'
          I = I + 1
          ...
          VINFO_T3D(I) = 'TURBULENT PRODUCTION K'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.AKN'
          VINFO_T3D(I) = 'TURBULENT PRODUCTION K AT TN'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.EP'
          VINFO_T3D(I) = 'TURBULENT DISSIPIATION EPS'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.EPN'
          VINFO_T3D(I) = 'TURBULENT DISSIPIATION EPS AT TN'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.RUGOF'
          VINFO_T3D(I) = 'FRICTION COEFFICIENT'
          I = I + 1
          VNAME_T3D(I) = 'MODEL.WIND'
          VINFO_T3D(I) = 'VELOCITY OF THE WIND'
          I = I + 1

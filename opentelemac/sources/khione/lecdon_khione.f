...
!     DECODES 'SUBMIT' CHAINS
      CALL READ_SUBMIT(ICE_FILES,MAXLU_ICE,FILE_DESC,300)
!
...
!
!
      CNI_MIN = MOTREA(ADRESS(2,50))    ! (0.02) MINIMUM ICEROUGHNESS
      CNISLD = MOTREA(ADRESS(2,51))     ! (0.02) ICE ISLAND ROUGHNESS
      CNI_MAX = MOTREA(ADRESS(2,52))    ! (0.06) MAXIMUM ICE ROUGHNESS
...
!      CNI = MOTREA(ADRESS(2,65))         ! => CNI_MIN
!      CNIMAX = MOTREA(ADRESS(2,67))      ! => CNI_MAX

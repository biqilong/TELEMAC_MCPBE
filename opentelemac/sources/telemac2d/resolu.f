...
!
     ...
     &  SPHERIC,SINLAT,LEO,NREG,TNP,PT_IN_POLY)
!
!***********************************************************************
! TELEMAC2D   V7P2
!***********************************************************************
...
!
      ...
      DOUBLE PRECISION, INTENT(IN)    :: AIRE(NELEM),DTHAUT(NPOIN)
...
!
      ...
      DOUBLE PRECISION XNC,W1,DMIN,BETA,TEST,GPRDTIME,RESTE,ATT
!
...
!     ==================================================================
      IF(LT.EQ.1) THEN
        CALL OV('X=YZ    ', X=QU, Y=HN, Z=U, DIM1=NPOIN)
        CALL OV('X=YZ    ', X=QV, Y=HN, Z=V, DIM1=NPOIN)
        DO I=1,NPOIN
          FLUX_OLD(I,1) = 0.0D0
          FLUX_OLD(I,2) = 0.0D0
          FLUX_OLD(I,3) = 0.0D0
...
!     ==================================================================
      IF(LT.EQ.1) CALL ENTETE(1,AT,LT,ASCHEME=ICIN,AORDRE=NORDRE)
!
      IF(ICIN .EQ.0) THEN
!-----------------------------------------------------------------------
!        ROE SCHEME
...
!
        CALL CALDT(NPOIN,G,HN,U,V,DTHAUT,DT,ATT,TMAX,
...
!
     ...
     &              NPOIN,X,Y,ZF,EPS,DMIN,G,
     &              XNEBOR,YNEBOR,LIMPRO,NBOR,KDIR,KNEU,KDDL,FLBOR,
     &              ELTSEG,IFABOR,MESH)
!
! INTEGRATION IN TIME
!
        CALL INTEMP(W,FLUX,FLUX_OLD,AIRS,DT,NPOIN,CF,EPS,KFROT,
...
!        CALCULATION OF SECOND MEMBER FOR TRACER
          CALL SMTRAC(NPOIN,DIMT,DT,SMTR%ADR(ITRAC)%P%R,
...
!
      ...
      ENDIF
!
!   FORCE GRAPHICAL OUTPUT TO SKIP TRACER COMPUTATION
      IF(NTRAC.GT.0.AND.LEO.AND.NORDRE.EQ.2)GOTO 200
!
!-----------------------------------------------------------------------
...
! TIME STEP UNDER CFL CONDITION (ORDRE 1)
!
      CALL CALDT(NPOIN,G,H,U,V,DTHAUT,DTN,ATT,TMAX,
     &           CFLWTD,ICIN,DTVARI,LISTIN,LEO,NORDRE)
!
...
!
      DO ITRAC=1,NTRAC
!
      CALL MAJTRAC(NPOIN,NELMAX,DIMT,NSEG,NPTFR,NUBO,
...
!
     ...
     &              W,FLUX,FLUENT,FLUSORT,FLBOR,EPS,

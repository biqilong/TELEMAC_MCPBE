...
!
      ...
      USE DECLARATIONS_TELEMAC2D, ONLY:CF,NPOIN,LT,KFROT,SPHERI,
     &                            CORIOL,GRAV,DT,FCOR,HN,QU,QV,
     &                            MESH,FU,FV
      IMPLICIT NONE
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
!
     ...
     &     (CF%R(IS).LE.1.D-12)) THEN
          AKAP = 1.D0
        ELSE
          IF(KFROT.GE.1.AND.KFROT.LE.5) THEN
          AKAP = 1.D0 + CF%R(IS)*DT*SQRT(QU%R(IS)**2+QV%R(IS)**2)/
     &           (2.D0*HN%R(IS)*UA(1,IS))
          ELSEIF(KFROT.EQ.0) THEN
            AKAP = 1.D0
            CF%R(IS) = 0.D0
          ELSE
            WRITE(LU,25)
25          FORMAT(1X,'FRICTION LAW NOT TAKEN INTO ACCOUNT ')
            CALL PLANTE(1)
            STOP
          ENDIF
        ENDIF

...
!
     ...
     & NELEM,NSEG,NPTFR,NPOIN,X,Y,ZF,EPS,DDMIN,G,
...
!
      ...
      DOUBLE PRECISION, INTENT(IN) :: ZF(NPOIN),VNOIN(3,*)
...
!
      ...
      CALL CHECK_ALLOCATE(IER, 'FLUX_TCH:YESNO')
!
      DO  NB =  1 , NPOIN
        FLUX(NB,1) = 0.D0
        FLUX(NB,2) = 0.D0
...
!
            IF(INDIC.LT.2) THEN
              ...
              YN = VNOIN (2,NSG)
!
              CALL FLUXE(HJ,UJ,VJ,HI,UI,VI,XN,YN,G,FLULOC)
              CALL FLUSRC(NUBO1,NUBO2,NSG,VNOIN,W,FLUSCE,X,Y,NPOIN,
     &                 NSEG,ZF,EPS,G)
            ELSE
              FLULOC(1)= 0.D0
              FLULOC(2)= 0.D0
...
!
            IF(INDIC.LT.2) THEN
             CALL FLUXE(HI,UI,VI,HJ,UJ,VJ,XN,YN,G,FLULOC)
! RA

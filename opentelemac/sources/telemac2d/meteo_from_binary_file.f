...
!
      USE BIEF
      USE DECLARATIONS_WAQTEL,ONLY: TAIR, ATMOSEXCH
!
...
!       SET THE WIND VELOCITY FROM THE ATMOSPHERIC BINARY FILE
     ...
     &      FILES(ATMFILE)%FMT,NPOIN,READ_BIN_WU)
        CALL READ_BIN_2D
     &     (WINDY,V_NAME,AT,FILES(ATMFILE)%LU,
     &      FILES(ATMFILE)%FMT,NPOIN,READ_BIN_WV)
     ...
     &   FILES(ATMFILE)%FMT,NPOIN,READ_BIN_AIRP)
     ...
     &   FILES(ATMFILE)%FMT,NPOIN,READ_BIN_AIRT)

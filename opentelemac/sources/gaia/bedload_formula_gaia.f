...
!
     ...
     & COEFPN,CALFA,SALFA,BIJK,HOULE,H_TEL,
...
!     ==================================================
     ...
     &        DCLA, AC, T1, T2, T3, HIDING, QSC,XMVS)
        DO I=1,NPOIN
          QSC%R(I)=QSC%R(I)*RATIO_SAND(I)
        ENDDO
!
...
!
     ...
     &        T8,T9,T10,T11,QSC,QSS,HOULE,XMVS)
        DO I=1,NPOIN
          QSC%R(I)=QSC%R(I)*RATIO_SAND(I)*HIDING%R(I)
          IF(.NOT.SUSP) THEN
            QSS%R(I)=QSS%R(I)*RATIO_SAND(I)*HIDING%R(I)
...
!
     ...
     &        T5, T6, T7, T8, T9, T10, T11, QSC,XMVS)
!       ARBITRARY DISTRIBUTION
        DO I=1,NPOIN
          QSC%R(I)=QSC%R(I)*RATIO_SAND(I)*HIDING%R(I)
        ENDDO

...
!       ALLOCATE ESTRATUM AND RATIO_INIT
        IF(.NOT.ALLOCATED(ESTRATUM)) THEN
          ALLOCATE(ESTRATUM(NUMSTRAT,NPOIN), STAT=IERR)
          IF(IERR.NE.0) THEN
            WRITE(LU,*)'ERROR REALLOCATING ESTRATUM:',IERR
            CALL PLANTE(1)
            STOP
          ENDIF
        ENDIF
        IF(.NOT.ALLOCATED(RATIO_INIT)) THEN
          ALLOCATE(RATIO_INIT(NSICLA,NUMSTRAT,NPOIN), STAT=IERR)
          IF(IERR.NE.0) THEN
            WRITE(LU,*)'ERROR REALLOCATING RATIO_INIT:',IERR
            CALL PLANTE(1)
            STOP
          ENDIF
        ENDIF
...
!
          DO IPOIN=1,NPOIN
!           OPTION FOR ACTIVE LAYER = 3 * DM IS MISSING (MAYBE IN HIRANO?)
            ELAY%R(IPOIN)= ELAY0
!
...
!           BECAUSE AT THE MOMENT IT IS ACTUALLY CONSTANT IN SPACE
            ELAY%R(IPOIN)= ELAY0
            IF (.NOT.CONC_MUD_FOUND) THEN

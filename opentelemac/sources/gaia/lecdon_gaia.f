...
!     IT CAN BE DIFFERENT BY MISTAKE)
      ...
      ENDIF
      SLVSED%SLV    = MOTINT( ADRESS(1, 26) )
      SLVSED%KRYLOV = MOTINT( ADRESS(1, 27) )
      SLVSED%PRECON = MOTINT( ADRESS(1, 28) )
      SLVSED%NITMAX = MOTINT( ADRESS(1, 29) )
...
! For the MCPBE flocculation model 
      FRACDIM_MAC = MOTREA(ADRESS(2, 81))
      FRACDIM_MEG = MOTREA(ADRESS(2, 82))
      FLOCMIC_DIAFIX = MOTREA(ADRESS(2, 83))
      FLOCMEG_DIAFIX = MOTREA(ADRESS(2, 84))
      FLOCMIC_DEN = MOTREA(ADRESS(2, 85))
      BRKFRAC_P1 = MOTREA(ADRESS(2, 86))
      BRKFRAC_P2 = MOTREA(ADRESS(2, 87))
      BRKFRAC_F2 = MOTREA(ADRESS(2, 88))
      BRK_K1 = MOTREA(ADRESS(2, 89))
      BRK_K2 = MOTREA(ADRESS(2, 90))
      BRK_K3 = MOTREA(ADRESS(2, 91))
      AGG_ALPHA = MOTREA(ADRESS(2, 92))
      BRK_EFF = MOTREA(ADRESS(2, 93))
      BRK_FY = MOTREA(ADRESS(2, 94))
      BRK_Q = MOTREA(ADRESS(2, 95))
      KBOLZ = MOTREA(ADRESS(2, 96))
      TEMPSTD = MOTREA(ADRESS(2, 97))
...
!
      ...
      WRITE(LU,*) 'K ','NUM_ICLA_ISAND ','NUM_ISAND_ICLA ',
     &         'NUM_ICLA_IMUD ','NUM_IMUD_ICLA ','NUM_ISUSP_ICLA'
      DO K=1,NSICLA
        WRITE(LU,*) K,NUM_ICLA_ISAND(K),NUM_ISAND_ICLA(K),
     &        NUM_ICLA_IMUD(K),NUM_IMUD_ICLA(K),NUM_ISUSP_ICLA(K)
      ENDDO
      WRITE(LU,*) 'NSUSP_TEL ',NSUSP_TEL
      IF(NSAND.GT.0) THEN
...
1402  FORMAT('FOR THE BIJKER REFERENCE CONCENTRATION',1I3,/,1X,
     ...
     &       'BEDLOAD = YES')
      ...
      ENDIF
!
!     CHECKS CONGAITENCY OF BEDLOAD LAWS
!

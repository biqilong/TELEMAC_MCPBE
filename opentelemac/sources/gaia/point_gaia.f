...
!
      ...
      ! 2/ LOCAL VARIABLES
      ! ------------------
      INTEGER :: I,K,NTR,IELM0,IELM1,IELBT,IELM0_SUB,CHECK_NVAR,NTCLA
...
! ? ALLOCATE ONLY IF BED_MODEL = 1?
      ...
      ALLOCATE(CONC_MUD_ACTIV_LAYER(NPOIN))
      ALLOCATE(MASS_MUD_ACTIV_LAYER(NPOIN))
...
!
     ...
     &                          1,'RATIOS',0,1,0,MESH)
      DO K=1,NOMBLAY
        DO I=1,NSAND
          DEALLOCATE(RATIOS%ADR(K+(I-1)*NOMBLAY)%P%R)
          RATIOS%ADR(K+(I-1)*NOMBLAY)%P%R=>RATIO_SAND(I,K,1:NPOIN)
          RATIOS%ADR(K+(I-1)*NOMBLAY)%P%MAXDIM1=NPOIN
          RATIOS%ADR(K+(I-1)*NOMBLAY)%P%DIM1=NPOIN
        ENDDO
      ENDDO
!
     ...
     &                          1,'RATIOM',0,1,0,MESH)
      DO K=1,NOMBLAY
        DO I=1,NMUD
          DEALLOCATE(RATIOM%ADR(K+(I-1)*NOMBLAY)%P%R)
          RATIOM%ADR(K+(I-1)*NOMBLAY)%P%R=>RATIO_MUD(I,K,1:NPOIN)
          RATIOM%ADR(K+(I-1)*NOMBLAY)%P%MAXDIM1=NPOIN
          RATIOM%ADR(K+(I-1)*NOMBLAY)%P%DIM1=NPOIN
        ENDDO
      ENDDO
!
...
!
      ...
      DO K=1,NOMBLAY
        ...
        LAYCONC%ADR(K)%P%MAXDIM1=NOMBLAY
        LAYCONC%ADR(K)%P%DIM1=NOMBLAY
      ENDDO
!
!     MASS_S
!
     ...
     &                          1,'MASS_S',0,1,0,MESH)
      DO K=1,NOMBLAY
        DO I=1,NSAND
          DEALLOCATE(MASS_S%ADR(K+(I-1)*NOMBLAY)%P%R)
          MASS_S%ADR(K+(I-1)*NOMBLAY)%P%R=>MASS_SAND(I,K,1:NPOIN)
          MASS_S%ADR(K+(I-1)*NOMBLAY)%P%MAXDIM1=NPOIN
          MASS_S%ADR(K+(I-1)*NOMBLAY)%P%DIM1=NPOIN
        ENDDO
      ENDDO
!
!     MASS_M
!
     ...
     &                          1,'MASS_M',0,1,0,MESH)
      DO K=1,NOMBLAY
        DO I=1,NMUD
          DEALLOCATE(MASS_M%ADR(K+(I-1)*NOMBLAY)%P%R)
          MASS_M%ADR(K+(I-1)*NOMBLAY)%P%R=>MASS_MUD(I,K,1:NPOIN)
          MASS_M%ADR(K+(I-1)*NOMBLAY)%P%MAXDIM1=NPOIN
          MASS_M%ADR(K+(I-1)*NOMBLAY)%P%DIM1=NPOIN
        ENDDO
      ENDDO
!
!     MTRANSFER ALLOCATED WITH SIZE 0 AND POINTING TO TRANSF_MASS
!
      CALL BIEF_ALLVEC_IN_BLOCK(MTRANSFER,NOMBLAY,1,'MTRANS',0,1,0,MESH)
      DO K=1,NOMBLAY
        DEALLOCATE(MTRANSFER%ADR(K)%P%R)
        MTRANSFER%ADR(K)%P%R=>TRANS_MASS(K,1:NPOIN)
        MTRANSFER%ADR(K)%P%MAXDIM1=NOMBLAY
        MTRANSFER%ADR(K)%P%DIM1=NOMBLAY
      ENDDO
!
!     TOCEMUD ALLOCATED WITH SIZE 0 AND POINTING TO TOCE_MUD
!
      CALL BIEF_ALLVEC_IN_BLOCK(TOCEMUD,NOMBLAY,1,'MTRANS',0,1,0,MESH)
      DO K=1,NOMBLAY
        DEALLOCATE(TOCEMUD%ADR(K)%P%R)
        TOCEMUD%ADR(K)%P%R=>TOCE_MUD(K,1:NPOIN)
        TOCEMUD%ADR(K)%P%MAXDIM1=NOMBLAY
        TOCEMUD%ADR(K)%P%DIM1=NOMBLAY
      ENDDO
!
!     PARTHE ALLOCATED WITH SIZE 0 AND POINTING TO PARTHENIADES
!
      CALL BIEF_ALLVEC_IN_BLOCK(PARTHE,NOMBLAY,1,'PARTHE',0,1,0,MESH)
      DO K=1,NOMBLAY
        DEALLOCATE(PARTHE%ADR(K)%P%R)
        PARTHE%ADR(K)%P%R=>PARTHENIADES(K,1:NPOIN)
        PARTHE%ADR(K)%P%MAXDIM1=NOMBLAY
        PARTHE%ADR(K)%P%DIM1=NOMBLAY
      ENDDO
!
     ...
     &     IELMT,1,2,MESH)
      ...
      NTCLA = NSICLA ! Number of total classes, including auxiliary classes, needed in MCPBE models
      IF (FLOC .AND. FLOC_TYPE.EQ.3) THEN
        IF (MCPBE_VER .EQ. 1) NTCLA=NSICLA+1 ! for 2CPBE
        IF (MCPBE_VER .GT. 1) NTCLA=NSICLA+2 ! for 3CPBE
        WRITE(LU,*) 'NO. OF DEPOSITION FLUXES ARE INCREASED TO ',NTCLA
      ENDIF

      CALL BIEF_ALLVEC_IN_BLOCK(FLUDP ,NTCLA,1,'FLUDP ',IELMT,1,2,MESH)
      CALL BIEF_ALLVEC_IN_BLOCK(FLUDPT,NTCLA,1,'FLUDPT',IELMT,1,2,MESH)
      CALL BIEF_ALLVEC_IN_BLOCK(FLUER ,NTCLA,1,'FLUER ',IELMT,1,2,MESH)
      CALL BIEF_ALLVEC_IN_BLOCK(FLUERT,NTCLA,1,'FLUERT',IELMT,1,2,MESH)
...
!
!     QSCL: FROM NVAR_QSCL+1 TO NVAR_CS
!
      DO I = 1, NSICLA
...
!
!     CS: FROM NVAR_CS+1 TO NVAR_QS_C
!
      DO I=1,NSICLA
...
!
      ...
      CHECK_NVAR=CHECK_NVAR+NOMBLAY*NSAND
!
!     MASS_M: FROM NVAR_MASS_M+1 TO NVAR_MTRANSFER
!
      DO I = 1,NOMBLAY*NMUD
      ...
      CHECK_NVAR=CHECK_NVAR+NOMBLAY*NMUD
!
!     MTRANSFER: FROM NVAR_MTRANSFER+1 TO NVAR_TOCEMUD
!
      DO I = 1,NOMBLAY

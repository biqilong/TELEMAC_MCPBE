...
! NUMBER OF LAYERS
      ...
      DO K=1,NOMBLAY
        ALLOCATE(MTRANSFER%ADR(K)%P%R(1))
      ENDDO
      CALL BIEF_DEALLOBJ(MTRANSFER)
      DO K=1,NOMBLAY
        ALLOCATE(TOCEMUD%ADR(K)%P%R(1))
      ENDDO
      CALL BIEF_DEALLOBJ(TOCEMUD)
      DO K=1,NOMBLAY
        ALLOCATE(PARTHE%ADR(K)%P%R(1))
      ENDDO
...
!
      ...
      DO I=1,NOMBLAY*NSAND
        ALLOCATE(RATIOS%ADR(I)%P%R(1))
        ALLOCATE(MASS_S%ADR(I)%P%R(1))
      ENDDO
      CALL BIEF_DEALLOBJ(RATIOS)
      CALL BIEF_DEALLOBJ(MASS_S)
      DO I=1,NOMBLAY*NMUD
        ALLOCATE(RATIOM%ADR(I)%P%R(1))
        ALLOCATE(MASS_M%ADR(I)%P%R(1))
      ENDDO
...
!
      ...
      DEALLOCATE(CONC_MUD_ACTIV_LAYER)
      DEALLOCATE(MASS_MUD_ACTIV_LAYER)

...
!     AVAI: FROM 24 TO 23+NOMBLAY*NSICLA
!
      DO I = 1,NOMBLAY*NSICLA
        CALL ADDBLO(VARSOR, AVAI%ADR(I)%P)
      ENDDO
!     QSCL: FROM 24+NOMBLAY*NSICLA TO 23+(NOMBLAY+1)*NSICLA
!
      DO I = 1, NSICLA
        CALL ADDBLO(VARSOR, QSCL%ADR(I)%P)
      ENDDO
!
!     CS: FROM 24+(NOMBLAY+1)*NSICLA TO 23+(NOMBLAY+2)*NSICLA
!
      DO I=1,NSICLA
        CALL ADDBLO(VARSOR, CS%ADR(I)%P)
      ENDDO
      CALL ADDBLO(VARSOR,QS_C)               ! 24+(NOMBLAY+2)*NSICLA
      CALL ADDBLO(VARSOR,QSXC)               ! 25+(NOMBLAY+2)*NSICLA
      CALL ADDBLO(VARSOR,QSYC)               ! 26+(NOMBLAY+2)*NSICLA
      CALL ADDBLO(VARSOR,QS_S)               ! 27+(NOMBLAY+2)*NSICLA
      CALL ADDBLO(VARSOR,QSXS)               ! 28+(NOMBLAY+2)*NSICLA
      CALL ADDBLO(VARSOR,QSYS)               ! 29+(NOMBLAY+2)*NSICLA
!
!     QSCL_C: FROM 30+(NOMBLAY+2)*NSICLA TO 29+(NOMBLAY+3)*NSICLA
!
      DO I=1,NSICLA
        CALL ADDBLO(VARSOR,QSCL_C%ADR(I)%P)
      ENDDO
!
!     QSCL_S: FROM 30+(NOMBLAY+3)*NSICLA TO 29+(NOMBLAY+4)*NSICLA
!
      DO I=1,NSICLA
        CALL ADDBLO(VARSOR,QSCL_S%ADR(I)%P)
      ENDDO
!
!     LAYTHI: FROM 30+(NOMBLAY+4)*NSICLA TO 29+(NOMBLAY+4)*NSICLA+NOMBLAY
!
      DO I=1,NOMBLAY
        CALL ADDBLO(VARSOR,LAYTHI%ADR(I)%P)
      ENDDO
!
!     CONC FROM  30+(NOMBLAY+4)*NSICLA+NOMBLAY TO 29+(NOMBLAY+4)*NSICLA+2*NOMBLAY
!
      DO I=1,NOMBLAY
        CALL ADDBLO(VARSOR,LAYCONC%ADR(I)%P)
      ENDDO
!
!     PRIVE: FROM 30+(NOMBLAY+4)*NSICLA+2*NOMBLAY TO
!                 29+(NOMBLAY+4)*NSICLA+MAX(4,NPRIV)+2*NOMBLAY
!
      DO I=1,MAX(4,NPRIV)

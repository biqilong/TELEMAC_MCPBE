Subroutine StabiliteTalus	(  &
	...
	Talus					,  & ! Paramï¿½tres relatifs aux talus
    Dt						,  & ! Pas de temps
...
use M_TALUS_T				! Dï¿½finition du type TALUS_T
...
! Variables locales
  ...
  integer :: NInt					! Nombre d'interfaces sï¿½dimentaires
  integer :: NPt					! Nombre de points dï¿½crivant le profil en travers
...
!  real(DOUBLE) :: Sec				! facteur de securite  ! PU2017 : Mise en commentaire
  ...
  real(DOUBLE), dimension(:,:), allocatable :: Z			! Cotes des interfaces sï¿½dimentaires
...
!=========================================================================
! Allocations des tableaux locaux dï¿½pendant de NPt
!=========================================================================
  ...
  ! Initialisation
	...
	! Dï¿½termination si le segment est immergï¿½ ou emerge
	  ImG = W1
	  ImD = W1
  ...
  ! Calcul de Dz, la deformation de la couche
	Do k = NInt-1, 1, -1
	  ...
	  Do j = 1, NPt-1
		...
		Else
	      Dz(j) = A(j) * (Z(k,j-1) - Z(k,j)) * Lam(j-1) +  C(j) * (Z(k,j+1) - Z(k,j)) * Lam(j)
		Endif
...
10	  Do j = j1, NPt
		...
		Elseif (((HsC + Dz(j)) < -EPSH) .and. (Dz(j) < -EPSH)) Then
		  ...
		  Do jj = jg, jd+1
			...
			Else
			  Dz(jj) = A(jj) * (Z(k,jj-1) - Z(k,jj)) * Lam(jj-1) + C(jj) * (Z(k,jj+1) - Z(k,jj)) * Lam(jj)
			Endif
		  Enddo
	...
	! Calcul de Dz definitif
	  Do j = 1, NPt
		Do kk = k, 1, -1	! le deplacement de la couche k entraine
							! une deformation des couches superieures
		  If (j == 1) Then			! Modif 3
		    DzPart(kk,j) = C(j) * (Z(kk,j+1) - Z(kk,j)) * Lam(j)	!???
		  Elseif (j == NPt) Then
		    DzPart(kk,j) = A(j) * (Z(kk,j-1) - Z(kk,j)) * Lam(j-1)	!???
		  Else
		    DzPart(kk,j) = A(j) * (Z(kk,j-1) - Z(kk,j)) * Lam(j-1) +  C(j) * (Z(kk,j+1) - Z(kk,j)) * Lam(j)
		  Endif

...
!  --------	  initialement présentes dans le bief
!
...
! ----------------------------------------------------------------------------
	...
	If (NbPtsConcIni < 0) Then
	  ...
	  Erreur%ft   = err_408
	  Erreur%ft_c = err_408c
	  call TRAITER_ERREUR (Erreur, 'Le nombre de points Concentration Init.',	&
								   '> ou egal a 0')
	  return
	End if
...
!chb			do iTraceur = 1, NbTraceurs
!chb				do iPts = 1, NbPtsConcIni
!chb					Traceurs(iTraceur,iPts) = MOTREA(ADRESS(2,5XX+iTraceur-1)+iPts-1)
!chb				end do	
!chb			end do
...
!=========================================================================
  ...
  If (ImpressionConcIni) Then
	...
	If(OptionCourlis) Then
	  ...
	  Do iPts = 1, NbProfCourlis
		write(UniteListing,10360) &
				Abscisse(iPts),(Traceurs0(iTraceur,iPts),iTraceur = 1, NbTraceurs)
	  End do
	End if
...
!=========================================================================
  ...
  10340 format (3A12)
  10350 format ('AbscIni  Traceurs(iTrac), iTrac = 1, NbTraceurs: ',A3)

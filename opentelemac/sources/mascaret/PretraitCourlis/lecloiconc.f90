...
!=========================================================================
  ...
  boucle_loi : Do iLoi = 1, NbLoiConc
	...
	! Nom de la loi
	!--------------
	LoiConc(iLoi)%Nom	= MOTCAR(ADRESS(4,607+iLoi-1))
	...
	! Nom du fichier contenant les donnees des lois de concentrations
	  ...
	  If (ImpressionLoiConc) Then
		write(UniteListing,2002) iLoi,trim(LoiConc(iLoi)%Nom),trim(FicLoiConc%Nom)
	  End if
	...
	Else If (ModeLoiConc == SAISIE_PAR_CLAVIER) Then
	...
	End if ModeSaisie		! FIN CHOIX MODE DE SAISIE
	...
	If (ImpressionLoiConc) Then
	  ...
	  Do iPts = 1, NbPts
		write(UniteListing,2005) iPts,LoiConc(iLoi)%Temps(iPts),LoiConc(iLoi)%Conc(iPts)
	  End do
	End if
